...
public class ByteOper {
	public static void main(String[] args) {
		 ...
		 * 常识：
		 * （1）二进制表示形式的第一位从0开始
		 * 	可以认为0是第一个位置的索引，索引都是从0开始，所以二进制的第一位的索引是0，第32位的索引是31。
		 * （2）二进制表示形式的第i位是从右往左数的
		 * 所以第1位是右边的起始位置
		 * （3）某一个二进制数X向左移动i位就会来到X所在的位置索引加上i的位置上，右移同理
		 * 比如1的二进制表示形式为：最右边是1，前面全是0。那么1向左移动1位就来到了：1原来的位置索引0 + 向左移动的位数1 = 1的位置，即1来到了第1位；此时如果1继续往左移动1个位置，那么1回来到：1现在的位置索引1 + 移动的位数1 = 索引为2的位置。
		 * 再比如二进制表示形式的第5位是1其它全是0，现在1往右移动3位，那么1来到了：1原来的索引位置5 - 移动位数3 = 索引位2的位置，注意索引为2的位置是第3位。
		 * （4）二进制换算成10进制
		 * 抛开符号位不说，哪一位上是1，该位的值就是2^index，将所有位的2^index加一块就是这个二进制数所对应的的十进制
		 * （5）一个数左移n位，等同于乘以2的n次方，即num << 1等同于num * 2^1，即扩大一倍，num << 3等同于num * 2^3 = num * 8；同理，右移相当于除以2^n。
		 *
		 * int的范围为什么是[-2^31,2^31-1]？
		 * 首先，为什么int正数范围的最大值是2^31-1，而不是别的什么2^31或者2^32或者2^30呢？
		 * 1、首先int占几个字节？
		 * 4个字节，32位，所以只要是int类型的整数在内存中都会占用4个字节的存储空间，合32位：0000 0000 0000 0000 0000 0000 0000 0000，即只要是int范围内的数，任何一个数所占用的空间都是一样的，都是32位，哪怕是1，前面的31位也是预留给1的，和第一位的1是一起的。
		 ...
		 * 所以int的范围是[-2^31,2^31-1]。上面说这么多其实就是在解释2的n次方中的n为什么是31而不是32或者30，为什么负数边界没有减1，而正数边界要减1。所以，负数最小值的绝对值比正数最大值的绝对值大1，负数的个数一共是2^31个，非负数的个数是2^31个，所以int范围内包含整数的个数是2^32个。
		 * 
		 * 之所以要保留一位当做符号位是因为int表示的是有符号整数，既可以表示正数也可以表示负数，必须要预留一位来表示符号；如果是无符号整数那么就可以用满这32个位来表示正数，所以在表示正数时，无符号整数表示的范围更大。所以负数最小值的绝对值要比整数最大值的绝对值大1，也就是能表示负数的个数要比能表示整数的个数多一个，如果把0归入正数范畴那么int表示正负数的个数就一样了，即正负数各有2^31个数，所以整体加起来一共有2^31 * 2 = 2^32个数。
		 * 
		 * 另外，负数的二进制表示在计算机中存储的值所代表的的真正的值不是他存储的这个值，真正代表的值是在这个值得基础上在经过三步操作后得到的值，这三步是：
		 * 1、取出符号位1，该值只代表这个数是负数
		 * 2、符号位后面的所有位全部按位取反，得到一个数
		 * 3、用第二步得到的数再+1，得到除符号位之外真正值的二进制表示。如果+1后超出了位数的限制，那么高位截断。
		 ...
		 * 这要从两点说起：
		 * 1、十进制的算术运算（比如：加减乘除、取模等）到计算机里面都会将操作数转换成二进制来操作，而二进制没有加减乘除、取模等运算，只有&、|、^、~、<<、>>、>>>这些位运算，所以十进制的算数运算最终会被转换成二进制的位运算。
		 ...
		 * 为了提高运算性能，在运算的时候无论操作数是正还是负都统一走一个通用的运算逻辑，负数的存储必须要经过特殊的处理，就是现在看到的样子。
		 * 
