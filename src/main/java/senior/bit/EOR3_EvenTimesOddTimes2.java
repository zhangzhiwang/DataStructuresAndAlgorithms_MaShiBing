...
public class EOR3_EvenTimesOddTimes2 {
	...
	private static void findOddTimes2(int[] arr) {
		 ...
		 * （注意：异或运算的法则是”异为1“，这里说的两个操作数都是只有一位的二进制数，所以异或的结果不是0就是1，但是上面的a和b都是由若干个二进制位组成的，a ^ b != 0不能说明a ^ b的结果就是1，可以是任意数。）
		 * 我们不妨找出eor中最右边的1在哪里，这里就是前面的与题1，senior.bit.AND1.java，算法是：int tmp = eor & (-eor)，这里假设tmp的第三位是1，即：0000 1000。
		 * eor的第三位是1说明什么？因为eor的由a ^ b得来的，说明a和b的第三位有一个是1另一个是0。
		 ...
		 * 4、根据上面第3步的结论继续往下想：既然a被分到了第三位是1的那拨，那么在申请一个变量叫eor2，然后同样从头到尾遍历一遍arr数组，和第一次遍历数组不一样的是，
		...
		for (int i = 0; i < arr.length; i++) {
			/**
			 * 如何判断一个数的第三位是1呢？因为tmp的第三位是1，其它所有位上都是0，所以让一个数与tmp进行&操作的结果是不是0就可以知道这个数的第三位是不是1了。
