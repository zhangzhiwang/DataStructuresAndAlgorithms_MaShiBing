...
/**
 * 树的高度
 * 节点的高度：（从下往上数，从0开始）该节点到叶子结点的最长路径（边数）。树的高度等于根节点的高度。
 * 节点的深度：（从上往下数，从0开始）从根节点到该节点的边数。从上往下看，根节点到任意节点的通路只有一条，因为在树中不允许多个节点指向同一节点，所以不存在最长路径的说法。
 * 树的深度就是最深叶节点的深度。所以从整个树来看，树的高度等于树的深度。
