...
/**
 * 判断达标路径和
 * 题目：给定一棵树和一个sum值，判断这棵树是否至少存在一个路径，该路径和等于sum。
 * 解释：
 * 1、完整路径：从头结点出发到叶节点结束这是一个完整的路径，一棵树有多少个叶节点就有多少个完整路径
 * 2、路径和：一条完整路径上所有节点的和就是这路径的路径和
 * 
 * 思路：设置一个全局静态变量flag，当到达叶节点的之后判断路径和是否和sum相等，如果相等置flag为true。如果不是叶子节就将路径和往下级不是null的节点传递。
 * 
 * 
 * @author zhangzhiwang
