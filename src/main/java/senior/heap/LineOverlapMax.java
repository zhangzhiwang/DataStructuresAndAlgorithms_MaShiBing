...
/**
 * 线段最大重合条数
 * 题目：给定若干条线段，每个线段都有起止位置[start,end]，且起止位置都是整数，线段之间会有重叠且重叠部分的长度最少为1，求重叠线段的最大条数。
 * 解释：
 * 1、”叠部分的长度最少为1“的意思是只有某一个点重合不叫重合，比如两条线段：一条是[1,3]，另一条是[3,5]，这两条线段只有在3这个点重合且重合的线段长度是0，所以这两条线段认为没有重合。
 * 2、要清楚这道题求什么，求的是重叠线段的最大条数而不是重叠线段的起止位置。
 * 3、通过题目的已知条件可以推断出：重叠部分的起止位置肯定是线段的起止位置，只不过重叠线段的”起“和”止“可能不是同一条线段的，有可能是不同线段的”起“和”止“。
 * 比如三条线段：[1,10][2,12][3,15]，那么重叠最多的部分是[3,10]，重叠部分被重复了3次，所以最终答案是3，单只这个重复的最多的部分的起止位置分别来自两条线段。
 * 
 * 思路：
 * 1、将所有线段的起始位置按照从小到大排序
 * 2、准备一个小根堆
 * 3、遍历排好序的所有线段，将每一个线段的起始位置和小根堆里面的元素作比较，所有小于等于这个起始位置的元素都被弹出，然后将该线段的结束为止放到堆里
 * 堆里面剩下的元素个数就是经过以该线段的起点为起点的重复线段的线段数，然后将线段的终点放入堆中。
 * 4、找出所有线段数的最大值
