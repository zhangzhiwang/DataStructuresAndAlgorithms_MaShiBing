...
public class ArrayKMTimes {
	public static void main(String[] args) {
		// 人工验证
		//		int[] arr = { 1, 1, 3, 3, 3, 6, 6, 6, 9, 9, 9 };
		//		int k = 2;
		//		int m = 3;
		//		int result = findOnlyKTimesValue(arr, k, m);
	 ...
	 * 找出出现k次的元素
	 * 思路：
	 * 1、题目给定的是int类型，所以准备一个int[]数组tmpArr，由于int类型的长度是32位，所以这个数组的额长度是32位，即int[] tmpArr = new int[32];
	 ...
	 * 假如tmpArr[0] % m = 0，并且假设出现k次的数是a，说明a的第0位不包含1；如果tmpArr[0] % m != 0，说明a的第0位一定包含1，以此类推判断所有的32位，这样就知道了a的32位哪些位上有1哪些没有。
	 ...
	 */
	public static int findOnlyKTimesValue(int[] arr, int k, int m) {
		if (arr == null) {
