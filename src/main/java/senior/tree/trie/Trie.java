...
public class Trie {
	...
	public void add(String value) {
		...
		// 注意：这为什么用临时变量替代root而不是用root直接操作？是因为下面curNode要随时移动，如果用root直接操作的话，那么Trie类的同一个对象再次调用add方法时root就已经不是根对象了额
	...
	public void delete(String value) {
		...
		if (!contains(value)) {// 如果不包含该字符串，直接退出，这一步很重要，不能省略，因为后面涉及pass的--以及节点的删除，如果遍历一半发现不包含该字符串就麻烦了，因为之前的节点pass已经--了。
			return;
		}
		...
		curNode.pass--;
		for (int i = 0; i < charArray.length; i++) {
			TrieNode curChar = curNode.tnMap.get((int) charArray[i]);// curChar肯定不为null
			if (--curChar.pass == 0) {
